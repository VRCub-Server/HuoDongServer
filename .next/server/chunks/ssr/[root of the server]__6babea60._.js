        ...
        const defaultEvents = [
            {
                ...
                cdKey: 'SURVIVAL2024',
                imageUrl: `https://placehold.co/800x400.png?text=Survival+Games`,
                organizerId: 'Notch'
            },
            {
                id: 'event2',
                ...
                cdKey: 'BUILDMC2024',
                imageUrl: `https://placehold.co/800x400.png?text=Build+Contest`,
                organizerId: 'Notch'
            },
            {
                id: 'event3',
                ...
                cdKey: 'PARKOURX',
                imageUrl: `https://placehold.co/800x400.png?text=Parkour+Challenge`,
                organizerId: 'Notch'
            },
            {
                id: 'event4',
                ...
                participants: [],
                cdKey: 'REDSTONEFAI',
                imageUrl: `https://placehold.co/800x400.png?text=Redstone+Expo`,
                organizerId: 'Notch'
            }
        ];
...
async function createUser(userData) {
    ...
    if (users.some((u)=>u.id === userData.id || u.username.toLowerCase() === userData.username.toLowerCase())) {
        console.warn(`Attempted to create a user that already exists: ${userData.username} (ID: ${userData.id})`);
...
async function /*#__TURBOPACK_DISABLE_EXPORT_MERGING__*/ login(prevState, formData) {
    ...
    if (!user) {
        ...
        try {
            // For new user, ID is the username. op defaults to 0 in apiCreateUser.
            const newUserBase = {
                id: minecraftUsername,
                username: minecraftUsername,
                avatarUrl: `https://placehold.co/100x100.png?text=${minecraftUsername.substring(0, 2).toUpperCase()}`
            };
            user = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$data$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["createUser"])(newUserBase);
        } catch (error) {
            ...
            if (error.message === '用户已存在。') {
                ...
                if (!user) {
                    return {
                        message: `创建用户时发生意外错误，但用户似乎已存在但无法检索。`
                    };
                }
            } else {
                return {
                    message: `创建用户时出错: ${error.message}`
                };
            }
        }
    }
    if (user) {
        ...
        (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$client$2f$components$2f$navigation$2e$react$2d$server$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["redirect"])('/dashboard'); // Redirect to dashboard
    } else {
        return {
            message: '登录失败，无法找到或创建用户账户。'
        };
    }
}
...
async function /*#__TURBOPACK_DISABLE_EXPORT_MERGING__*/ getSessionUser() {
    ...
    try {
        return await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$data$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["getUserById"])(userId);
    } catch (error) {
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__6babea60._.js.map