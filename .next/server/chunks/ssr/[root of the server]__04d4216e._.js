...
async function createUser(userData) {
    ...
    if (existingUser) {
        ...
        // This case should ideally be handled by the action layer before calling createUser if a distinct error is needed for "user exists"
        return existingUser;
    }
    const newUser = {
    ...
    try {
        ...
        // Re-throw or handle as per application's error strategy. For now, re-throw to indicate failure.
...
async function getEventParticipants(eventId) {
    ...
    SELECT u.id, u.username, u.avatarUrl, u.op, u.password, ep.isCheckedIn
    FROM users u
    JOIN event_participants ep ON u.id = ep.userId
    WHERE ep.eventId = ?
  `);
    const participantsData = stmt.all(eventId); // password is included here
...
async function getEventsForUser(userId) {
    ...
    WHERE ep.userId = ?
  `);
    const eventIds = stmt.all(userId);
    const joinedEvents = [];
...
async function createEvent(eventData, organizerId) {
    ...
    // Ensure participantLimit is null if it's undefined or an empty string (or not a number)
...
async function updateEventById(eventId, eventData) {
    ...
    // Ensure participantLimit is null if it's undefined or an empty string (or not a number)
    const actualParticipantLimit = typeof participantLimit === 'number' && !isNaN(participantLimit) ? participantLimit : null;
    const stmt = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$db$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"].prepare('UPDATE events SET name = @name, date = @date, description = @description, participantLimit = @participantLimit, cdKey = @cdKey, imageUrl = @imageUrl WHERE id = @eventId');
    const result = stmt.run({
        name,
        date,
        description,
        participantLimit: actualParticipantLimit,
        cdKey,
        imageUrl,
        eventId
    });
    if (result.changes === 0) return null;
    const updatedEvent = await getEventById(eventId); // Fetch updated event
...
async function deleteEventById(eventId) {
    ...
    // CASCADE delete handles event_participants table
    if (result.changes > 0) {
        return {
            success: true,
            message: "活动已成功删除。"
...
async function joinEvent(userId, eventId, cdKeyInput) {
    ...
    const participants = await getEventParticipants(eventId); // Fetch current participants
    if (event.participantLimit && participants.length >= event.participantLimit) {
        return {
            success: false,
            message: '活动人数已满。'
        };
    ...
    if (participants.some((p)=>p.id === userId)) {
        const updatedEvent = await getEventById(eventId); // Re-fetch to ensure participants array is fresh
        return {
            success: true,
            message: '您已加入此活动。',
            event: updatedEvent
    ...
    const finalEvent = await getEventById(eventId); // Re-fetch to include the new participant
    return {
        success: true,
        message: '成功加入活动！',
        event: finalEvent
...
async function leaveEvent(userId, eventId) {
    ...
    if (result.changes > 0) {
        const updatedEvent = await getEventById(eventId); // Re-fetch to update participant list
        return {
            success: true,
            message: '成功离开活动。',
            event: updatedEvent
...
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$data$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/lib/data.ts [app-rsc] (ecmascript)"); // data.ts now uses SQLite
...
async function verifyGeetestV4(lot_number, captcha_output, pass_token, gen_time) {
    if (!("TURBOPACK compile-time value", "7087c8f06d892afd4d6e4334362d218b") || !process.env.GEETEST_SECRET_KEY) {
        console.error("Geetest CAPTCHA_ID 或 SECRET_KEY 未配置。");
        return false; // Or true in dev if you want to bypass
    }
    if (!lot_number || !captcha_output || !pass_token || !gen_time) {
        console.warn("Geetest 客户端参数缺失。");
        return false;
    }
    ...
    const sign_token = crypto.createHmac('sha256', process.env.GEETEST_SECRET_KEY).update(lot_number).digest('hex');
    ...
    try {
        ...
        console.log("Geetest 验证响应:", data);
        return data.status === 'success';
    } catch (error) {
        console.error('Geetest 验证 API 请求失败:', error);
        return false;
    }
}
async function /*#__TURBOPACK_DISABLE_EXPORT_MERGING__*/ authenticateUserAction(prevState, formData) {
    try {
        ...
        // Skip Geetest validation if ID/Key are not set, useful for local dev without Geetest setup.
        const geetestConfigured = !!("TURBOPACK compile-time value", "7087c8f06d892afd4d6e4334362d218b") && !!process.env.GEETEST_SECRET_KEY;
        if (geetestConfigured) {
            ...
            console.warn("Geetest环境变量未完全配置，跳过人机验证。");
        }
        if (authMode === 'login') {
...
const CreateEventSchema = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zod$2f$lib$2f$index$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["z"].object({
    ...
    participantLimit: __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zod$2f$lib$2f$index$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["z"].preprocess((val)=>val === "" || val === null || val === undefined ? undefined : Number(val), __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zod$2f$lib$2f$index$2e$mjs__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["z"].number().int().positive().optional().nullable() // Allow null for DB
    ),
...
async function /*#__TURBOPACK_DISABLE_EXPORT_MERGING__*/ joinEventUsingCdKeyOnlyAction(cdKey) {
    ...
    try {
        const result = await joinEventAction(eventToJoin.id, cdKey);
        return result;
    } catch (error) {
...
async function /*#__TURBOPACK_DISABLE_EXPORT_MERGING__*/ getUserProfile(userId) {
    ...
    const joinedEvents = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$data$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["getEventsForUser"])(userId); // Use the new function
    return {
        user: {
            ...user
        },
...
//# sourceMappingURL=%5Broot%20of%20the%20server%5D__04d4216e._.js.map